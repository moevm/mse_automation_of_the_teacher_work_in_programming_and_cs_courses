        ...
        try:
            for student in self.students:   # для каждого студента
                ...
                for i, course in enumerate(self.courses):   # для каждого курса
                    ...
                    if self.course_grades[i][student['id']]['steps']:  # если статистика прохождения студента не пуста (он поступил на курс)
                        for sect in course['sections']:  # для всех модулей курса
                            sect_date = date(1970, 1, 1)    # для определения последнего решения модуля, в случае его полного прохождения
                            ...
                            if correct_step_counter != step_counter:
                                ...
                                sect_date = sect_date.strftime("%d.%m.%Y")  # последнее верное решение в модуле, если он пройден полностью
                            sect.update({
                                'date': sect_date,
                        ...
                        course.update({'progress': self.course_grades[i][student['id']]['progress']})  # сохранение информцаии о модулях в курсе, если прогресс существует (студент поступил на курс)
                    else:
                        course.update({
                            'progress': 'Нет',
                            'sections': []
    ...
    def table_step_info(self, course_id):
        ...
        try:
            ...
            for student in self.students:
                ...
                student_table_rows = [self.short_step_info(step, student['id']) for sect in course['sections'] for lesson in sect['lessons'] for step in lesson['steps']]
